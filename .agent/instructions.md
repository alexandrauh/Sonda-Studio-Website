...
- Consider utility-first frameworks- **Tailwind CSS v4**: Modern styling with native CSS variables.
- **Storyblok CMS**: Visual editing with a "Manual Root" component pattern for stability.

## Core Patterns

### 1. Storyblok Integration
- **Manual Root Resolution**: Always manually import and resolve the `Page` component in page views instead of relying on the SDK's dynamic mapper for the top level.
- **Case-Insensitivity**: Components should handle both lowercase and capitalized field names using `computed` properties (e.g., `props.blok?.headline || (props.blok as any)?.Headline`).
- **Dynamic Children**: Allow `StoryblokComponent` to handle nested components within `Page.vue`, but maintain a manual "map" in `Page.vue` for known blocks to ensure reliable rendering.

### 2. Local Development Environment
- **HTTPS Required**: The development server MUST use HTTPS (via `@vitejs/plugin-basic-ssl`) because Storyblok's visual editor requires a secure context to load the site in an iframe.
- **URL Handshake**: For local development, the user must manually visit the `https://localhost:5173` URL once in a separate tab to "Trust" the local SSL certificate before it will load in Storyblok.
- Leverage CSS custom properties for theming and design tokens (colors, fonts, spacing).
- ALWAYS use variables for colors, fonts, and spacing. Avoid hardcoded values (e.g., `text-[9rem]`, `#f4efe0`).
...
- **Mobile-First**: Always start with mobile styles and use `@media (min-width: ...)` for larger screens.
- **Fluid Typography**: Use `clamp()` for large headlines to ensure they scale elegantly without overflowing.
- **Interactive Elements**:
    - All interactive elements (buttons, inputs, toggles) MUST have descriptive `aria-label` or associated `<label>`.
    - Use `aria-expanded` and `aria-pressed` to communicate state.
    - Focus states must be visible and clear (`focus-visible`).
- **Semantics**:
    - Use `aria-hidden="true"` for purely decorative elements (numbered indices, icons).
    - Use `.sr-only` for screen-reader-only labels where visual labels aren't appropriate.
- **Forms**:
    - Every input must have an `id` associated with a label's `for` attribute.
